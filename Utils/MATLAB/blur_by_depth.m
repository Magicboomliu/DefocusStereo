function [blurred_image, depth_map, camera_params] = blur_by_depth(image_file_name, depth_map_file_name, depth_scale, kernel_type, max_coc, is_gpu, g, camera_param)
    % get image
    image = get_image(image_file_name);
    %depth_map = (get_depth_map(depth_map_file_name, depth_scale)/1000-1.5)*1.3;
    depth_map2 = get_depth_map(depth_map_file_name, depth_scale);

    disp('------------');
    disp('Disp info');
    disp('------------');
    disp(['Disp max-min: ',num2str(max(depth_map2,[],'all')),' -- ', num2str(min(depth_map2,[],'all'))]);

    max_depth_layers = 450;
    min_depth_layers = 200;

    depth_map_origin = depth_map2;
    max_count = 20;
    count = 0;
    % compute CoCs
    while true
        [cocs, depth_map, camera_params] = get_cocs(depth_map_origin, size(image, 2), max_coc, camera_param);
    ...
    coc_zero_idx = find(cocs <= 0.1);
    coc_length = length(coc_zero_idx);
    if coc_length > 0
    ...
    for i = 1:max_idx
        ...
        if i > coc_zero_idx && coc_zero_idx ~= -1
...
%     isequal(depth_map, depth_map2)
%     all(depth_map(:) == depth_map2(:))
...
function depth_map = get_depth_map(depth_map_file_name, depth_scale)
    ...
    elseif strcmp(ext, '.pfm')
        depth_map = double(pfmread(depth_map_file_name)); 
...
function [cocs, depth_map, camera_params] = get_cocs(depth_map, I_width, max_coc, camera_param)
    %% camera parameters
    epsilon = 0.0001;
    camera_param;
